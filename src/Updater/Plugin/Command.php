    ...
    private function action()
    {
        return function () {
            ...
            try {
                ...
                exit(1);
            } catch (UpdateNotAvailableOnDevBuildException $e) {
                echo \sprintf('You are running a development build. The latest public release is: %s', $e->getLatestVersion()) . \PHP_EOL;
                exit(1);
            } catch (NoVersionDeterminedException) {
                echo 'Unable to determine your current workspace version. You are likely not using a tagged release.' . \PHP_EOL;
                exit(1);
